#ifndef PERIODLC_RT2_HPP
#define PERIODLC_RT2_HPP
...
public:
    ...
    RtTask2(float period_s, std::string name, long stack_size, int priority)
        : period_s_(period_s)
        , name_(name)
    ...
    void start(void)
    {
        ...
        is_running_ = true;
        thread_id_ = new std::thread(&RtTask2::loop_function, this);
    }
    ...
    void reset_max_period(void)
    {
        max_period_ = 0;
        return;
    }
    void reset_max_runtime(void)
    {
        max_runtime_ = 0;
        return;
    }
...
//线程任务集合管理器
//class RtTaskManager2 {
// public:
//  RtTaskManager2(void){};
//  ~RtTaskManager2(void) {}

//void add_task(RtTask2* task) { tasks_.push_back(task); }
...
//private:
// std::vector<RtTask2*> tasks_;
//};

//类成员函数的周期线程任务
template<typename T>
class RtPeriodicMemberFun2 : public RtTask2 {
public:
    RtPeriodicMemberFun2(float period_s, std::string name, long stack_size, int priority,
                         void (T::*function)(), T* obj)
        : RtTask2(period_s, name, stack_size, priority)
        , _function(function)
        , _obj(obj)
    {
    }

    ~RtPeriodicMemberFun2(void) { stop(); }
