...
ArmReportStatus::ArmReportStatus(Socket* socket_fp, int axis)
{
    arminit(socket_fp, axis);
}
...
void ArmReportStatus::arminit(Socket* socket_fp, int axis)
{
    ...
    recv_task_ = new RtPeriodicMemberFun<ArmReportStatus>(
        0.0005, "recv_task", 1024 * 512, 30, &ArmReportStatus::recv_proc, this);
    recv_task_->start();
}

void ArmReportStatus::close(void)
{
    is_error_ = true;
}

bool ArmReportStatus::is_error(void)
{
    return is_error_;
}
...
void ArmReportStatus::flush_data(uint8_t* rx_data, int len)
{
    ...
    HexData::hex_to_fp32(
        &rx_data[k + 17 + axis * 4 + 6 * 4], report_status_[report_flag_].tau, axis);
    rxcnt_++;
    is_update_ = true;
}
...
void ArmReportStatus::print_data(void)
{
    print_data(&report_status_[report_flag_]);
}