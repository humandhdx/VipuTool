...
public:
    ...
    int moveto_cartesian_lineb(float* mvpose, float mvvelo, float mvacc, float mvtime,
                               float mvradii);
    int moveto_cartesian_p2p(float* mvpose, float mvvelo, float mvacc, float mvtime);
    int moveto_cartesian_p2pb(void);
    int moveto_cartesian_circle(float* pose1, float* pose2, float mvvelo, float mvacc, float mvtime,
                                float percent);
    ...
    int moveto_joint_circle(float* mvjoint1, float* mvjoint2, float mvvelo, float mvacc,
    ...
    int set_pass_rs485_now(uint8_t line, uint8_t timeout_ms, uint8_t tx_len, uint8_t rx_len,
    ...
    int get_gpio_in(uint8_t line, uint8_t id, int32_t* fun, int32_t* digit, int* adc_num,
                    float* adc_value);
    int get_gpio_out(uint8_t line, uint8_t id, int32_t* fun, int32_t* digit, int* adc_num,
                     float* adc_value);
...
protected:
    ARM_REG* reg_ = nullptr;
    int is_error_ = false;
private:
    utrc_t utrc_tx_;
    utrc_t utrc_rx_;
    Socket* socket_fp_ = nullptr;
    UtrcClient* utrc_client_ = nullptr;
